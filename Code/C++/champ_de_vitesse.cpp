			...
			std::vector<std::vector<int>> champ_distances)
{
	...
	unsigned int t = champ_vitesses->size();
	int tt = t*t;
	// Copie de la matrice pour les calculs
	std::vector<std::vector<Vect>> copy(t, std::vector(t, Vect(0,0)));
	...
	for(unsigned int i=0; i<n; i++)
	{
		...
		if (!((x-1>0 && champ_distances[x-1][y]>tt) |
			  (x+1<t && champ_distances[x+1][y]>tt) |
			  (y-1>0 && champ_distances[x][y-1]>tt) |
			  (y+1<t && champ_distances[x][y+1]>tt) |
			  (x+1<t && y+1<t && champ_distances[x+1][y+1]>tt) |
			  (x-1>0 && y+1<t && champ_distances[x-1][y+1]>tt) |
			  (x+1<t && y-1>0 && champ_distances[x+1][y-1]>tt) |
			  (x-1>0 && y-1>0 && champ_distances[x-1][y-1]>tt)))
		{
